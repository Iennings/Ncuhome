        ...
        try {
            ...
            httpUtil.postl("http://www.ncuos.com/api/library/libinfo", json, sharedPreferences.getString("token", null), new Callback() {
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    ...
                    BindLibrary.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            jsonAnalysis.parseJSONWithLib(BindLibrary.this, responseStr);
                            num = sharedPreferences.getInt("bookStatus", 0);
                            if (num != 0) {
                                Intent intent = new Intent(BindLibrary.this, LibraryRentBook.class);
                                startActivity(intent);
                            }
                            else{
                                Toast.makeText(BindLibrary.this, "密码错误", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                        }
